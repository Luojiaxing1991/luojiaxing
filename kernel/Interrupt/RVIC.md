...
如果RVIC中存在一个或者多个pending的中断，会通过虚拟的中断异常（virtual IRQ exception）来告知vPE(存疑，目前这一块的处理机制尚不明确)。当vPE处理中断异常时，会向RVIC这里询问是哪个中断触发了异常。每一个RVIC中每一个中断都有一个独立的编号（INTID）来标识，这个INTIT是VM范围内独一无二的。（INTID？ GIC里面是有vINTID的，这个需要明确下）
